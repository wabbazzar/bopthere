...
# CORS configuration
ALLOWED_ORIGINS = os.environ.get('ALLOWED_ORIGINS', 'https://heatherandwesley.com,http://localhost:5173').split(',')
...
def get_top_scores(game, limit=10):
    ...
    try:
        ...
        all_scores = table.query(
            KeyConditionExpression=Key('game').eq(game)
        )
        unique_players = set(item.get('username') for item in all_scores.get('Items', []))
...
def submit_score(game, username, score, character):
    ...
    try:
        ...
        score_timestamp = f"{999999999 - int(score):09d}#{datetime.utcnow().isoformat()}Z"
