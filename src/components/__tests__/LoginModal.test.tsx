...
jest.mock('@/components/ui/dialog', () => ({
  ...
  DialogTitle: ({ children, style, className }: { children: React.ReactNode; style?: React.CSSProperties; className?: string }) => 
    <h2 data-testid="dialog-title" style={style} className={className}>{children}</h2>,
}));
...
jest.mock('@/components/ui/button', () => ({
  Button: ({ children, onClick, disabled, type, style, className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { 'data-testid'?: string }) => 
    <button 
...
jest.mock('@/components/ui/input', () => ({
  Input: ({ onChange, value, disabled, placeholder, type, id, style, className, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { 'data-testid'?: string }) => 
    <input
...
describe('LoginModal', () => {
  ...
  describe('Authentication Flow', () => {
    ...
    it('should show loading state during authentication', async () => {
      const mockLogin = jest.fn((): Promise<void> => new Promise(resolve => setTimeout(resolve, 100)));
      renderLoginModal({}, { login: mockLogin });
