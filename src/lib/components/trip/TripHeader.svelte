	...
	<div class="flex items-baseline justify-between gap-3">
		{#if editingField === 'name'}
			<input
				...
				class="font-display text-2xl md:text-3xl font-semibold flex-1 min-w-0 bg-transparent"
				style="border: none; border-bottom: 2px solid var(--accent); outline: none; color: var(--ink)"
			/>
		{:else}
			<!-- svelte-ignore a11y_no_static_element_interactions -->
			<h1
				class="font-display text-2xl md:text-3xl font-semibold cursor-text flex-1 min-w-0 truncate"
