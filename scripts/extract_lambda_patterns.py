...
class LambdaPatternExtractor:
    ...
    def create_api_gateway_event(self, method: str, path: str, body: Optional[Dict] = None, 
    ...
    def invoke_lambda(self, function_name: str, event: Dict[str, Any]) -> Tuple[int, Dict[str, Any]]:
    ...
    def test_rsvp_endpoints(self, function_name: str) -> List[Dict[str, Any]]:
        ...
        rsvp_id = None
        if status == 200 and isinstance(response, dict) and response.get('statusCode') == 200:
        ...
        if rsvp_id:
            ...
            event = self.create_api_gateway_event("GET", f"/rsvp/{rsvp_id}", 
                                                path_params={"id": rsvp_id})
            status, response = self.invoke_lambda(function_name, event)
            test_cases.append({
        ...
        logger.info("\n=== Test 6: GET with invalid ID ===")
        event = self.create_api_gateway_event("GET", "/rsvp/invalid-uuid-12345", 
                                            path_params={"id": "invalid-uuid-12345"})
        status, response = self.invoke_lambda(function_name, event)
        test_cases.append({
        ...
        patterns = {
            "request_patterns": {
                "POST /rsvp": {
                    ...
                    "optional_fields": ["phone", "notifications", "dietary_restrictions", 
                                      "song_request", "message_for_couple"],
                    "field_types": {
                        "name": "string",
                        "email": "string",
                        "phone": "string",
    ...
    def generate_api_documentation(self, lambda_config: Dict[str, Any], 
    ...
    def extract(self, function_name_pattern: str = 'heatherandwesley-rsvp-handler'):
        ...
        if not lambda_func:
            logger.error(f"Lambda function containing '{function_name_pattern}' not found")
            return
