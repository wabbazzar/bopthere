...
class APIGatewayExtractor:
    ...
    def get_resources(self, api_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.api_gateway.get_resources(restApiId=api_id, limit=500)
    ...
    def get_integration(
        ...
        try:
            ...
            logger.error(
                f"Error fetching integration for {http_method} {resource_id}: {e}"
            )
            return None
    ...
    def extract_routes(
        self, api_name_pattern: str = "heatherandwesley", default_stage: str = "prod"
    ) -> Dict[str, Any]:
        ...
        api_info = {
            ...
            "api_id": api_id,
            "api_type": "REST",
            "created_date": api.get("createdDate").isoformat()
        ...
        try:
            ...
            api_info["deployments"] = [
                {
                    "id": d["id"],
                    "created_date": d.get("createdDate").isoformat()
            ...
            api_info["stages"] = [
                {
                    ...
                    "created_date": s.get("createdDate").isoformat()
                    if s.get("createdDate")
                    else None,
                    "last_updated_date": s.get("lastUpdatedDate").isoformat()
                    if s.get("lastUpdatedDate")
                    else None,
                }
            ...
            logger.info(
                f"Found {len(api_info['deployments'])} deployments and {len(api_info['stages'])} stages"
            )
            ...
            if not api_info["stages"] and api_info["deployments"] and default_stage:
                logger.info(
                    f"No stages found via API, adding default stage '{default_stage}'"
                )
                api_info["stages"] = [
                    {
                        "name": default_stage,
                        "deployment_id": api_info["deployments"][0]["id"]
                        if api_info["deployments"]
                        else None,
            ...
            for http_method, method_info in methods.items():
                ...
                if integration:
                    ...
                    if integration.get("requestParameters"):
                        route_info["integration"]["request_parameters"] = integration[
                            "requestParameters"
                        ]
                    ...
                    if lambda_arn:
                        ...
                        if ":function:" in lambda_arn:
                            function_name = lambda_arn.split(":function:")[1].split(
                                ":"
                            )[0]
                            route_info["integration"][
