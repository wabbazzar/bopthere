...
class DynamoDBSchemaExtractor:
    ...
    def get_table_schema(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = table.scan(Limit=100)  # Scan up to 100 items for schema discovery
            ...
            for item in items:
                ...
                for field_name, value in item.items():
                    ...
                    if len(field_info[field_name]['examples']) < 3 and value is not None:
                    ...
                    if not field_info[field_name]['description']:
                        field_info[field_name]['description'] = self._get_field_description(field_name)
            ...
            schema = {
                ...
                'keySchema': {
                    'partitionKey': next(k['AttributeName'] for k in key_schema if k['KeyType'] == 'HASH'),
                    'sortKey': next((k['AttributeName'] for k in key_schema if k['KeyType'] == 'RANGE'), None)
                },
                'attributes': {
            ...
            logger.info(f"Successfully extracted schema for {table_name} with {len(fields_schema)} fields")
            return schema
    ...
    def generate_markdown_docs(self, schemas: Dict[str, Any]) -> str:
        ...
        for table_name, schema in schemas.items():
            ...
            for field_name, field_info in sorted_fields:
                ...
                md_lines.append(f"| `{field_name}` | {types} | {required} | {description} |")
            ...
            for field_name, field_info in sorted_fields:
                if field_info['examples']:
                    examples = ', '.join([f"`{ex}`" for ex in field_info['examples'][:3]])
