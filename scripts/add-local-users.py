...
spec = importlib.util.spec_from_file_location("seed_users", os.path.join(os.path.dirname(__file__), "seed-users.py"))
    ...
    try:
        ...
        for i, user_data in enumerate(users_data):
            try:
                ...
                if missing_fields:
                    logger.error(f"User {i+1}: Missing required fields: {', '.join(missing_fields)}")
                ...
                # Create user
                if args.dry_run:
                    logger.info(f"[DRY RUN] Would create user: {user_data['username']} ({user_data['role']})")
                    # Test password hashing
                    test_hash = seeder._hash_password(user_data['password'])
                    if seeder._verify_password(user_data['password'], test_hash):
                        logger.info(f"[DRY RUN] Password hashing verification: PASSED for {user_data['username']}")
                    else:
                        logger.error(f"[DRY RUN] Password hashing verification: FAILED for {user_data['username']}")
                else:
                    ...
                    logger.info(f"✓ Successfully created user: {user_data['username']} ({user_data['role']})")
                
            except Exception as e:
                logger.error(f"Failed to create user {user_data.get('username', f'user_{i+1}')}: {e}")
                failed_users.append(user_data.get('username', f'user_{i+1}'))
                continue
        ...
        if args.dry_run:
            logger.info(f"\n[DRY RUN] Summary:")
            logger.info(f"  Users that would be created: {len(users_data) - len(failed_users)}")
        ...
        if not args.dry_run and created_users:
            ...
            for user_data in users_data:
                if user_data['username'] not in failed_users:
                    ...
                    is_valid = seeder._verify_password(user_data['password'], stored_hash)
