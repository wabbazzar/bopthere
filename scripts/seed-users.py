...
class UserSeeder:
    ...
    def create_auth_table(self, table_name: str, dry_run: bool = False) -> bool:
        ...
        if dry_run:
            ...
            logger.info(
                "[DRY RUN]   Fields: username, password_hash, email, full_name, role, created_at, last_login"
            )
            return True
        ...
        try:
            ...
            table_definition = {
                ...
                "Tags": [
                    {"Key": "Name", "Value": "Wedding App Authentication Users"},
                    {
                        "Key": "Description",
            ...
            waiter.wait(
                TableName=table_name, WaiterConfig={"Delay": 5, "MaxAttempts": 120}
            )
    ...
    def create_user(
        ...
        if dry_run:
            ...
            if self._verify_password(password, password_hash):
                logger.info(
                    f"[DRY RUN] Password hashing verification: PASSED for {username}"
                )
            else:
                logger.error(
                    f"[DRY RUN] Password hashing verification: FAILED for {username}"
                )
        ...
        for user_data in default_users:
            try:
                created_user = self.create_user(
                    table_name=table_name, dry_run=dry_run, **user_data
                )
    ...
    def seed_custom_users(
        self, table_name: str, users_file: str, dry_run: bool = False
    ) -> List[Dict[str, Any]]:
        """
            ...
            for user_data in users_data:
                ...
                for field in required_fields:
                    if field not in user_data:
                        raise ValueError(
                            f"Missing required field '{field}' in user data"
                        )
    ...
    def list_users(self, table_name: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in response.get("Items", []):
                ...
                safe_item = self._decimal_to_float(
...
Supported Roles:
  ...
  - admin: Administrative access
    ...
    # Seeding options
    parser.add_argument(
        "--users-file", help="JSON file containing custom users to seed"
    )
    ...
    parser.add_argument(
        "--skip-default", action="store_true", help="Skip creating default test users"
    )
    ...
    # Query options
    parser.add_argument(
        "--list-users", action="store_true", help="List existing users in the table"
    )
    ...
    try:
        ...
        if args.list_users:
            ...
            if not users:
                ...
                for i, user in enumerate(users):
                    ...
                    logger.info(
                        f"  - {username} ({role}) - {email} - created: {created_at}"
                    )
        ...
        if not args.skip_default:
            ...
            created_users = seeder.seed_default_users(
                args.table_name, dry_run=args.dry_run
            )
            total_created += len(created_users)

            if args.dry_run:
                logger.info(
                    f"[DRY RUN] Would create {len(created_users)} default users"
                )
            else:
        ...
        if args.users_file:
            ...
            created_users = seeder.seed_custom_users(
                args.table_name, args.users_file, dry_run=args.dry_run
            )
            total_created += len(created_users)
        ...
        if args.dry_run:
            ...
            logger.info(
                f"User seeding completed! Total users processed: {total_created}"
            )
