...
class TestAPIGatewayExtractor:
    ...
    def test_extract_routes_success(self, mock_boto3_session):
        ...
        # Mock API discovery
        mock_client.get_rest_apis.return_value = {
            'items': [{'id': 'api123', 'name': 'heatherandwesley-api', 'createdDate': datetime.utcnow()}]
        }
        ...
        mock_client.get_resource.side_effect = [
            ...
            {'resourceMethods': {'POST': {'authorizationType': 'NONE'}}}  # /rsvp has POST
        ]
        ...
        assert result['routes'][0]['integration']['lambda_function_name'] == 'rsvp-handler'
...
class TestLambdaPatternExtractor:
    ...
    def test_test_rsvp_endpoints(self, mock_boto3_session):
        ...
        responses = [
            ...
            {'statusCode': 400, 'body': json.dumps({'error': 'Missing required field: email'})},
            ...
            # Unsupported method
            {'statusCode': 405, 'body': json.dumps({'error': 'Method PUT not allowed'})},
            # Malformed JSON
    ...
    def test_save_patterns(self, mock_mkdir, mock_file, mock_boto3_session):
        ...
        json_calls = [call for call in mock_file.call_args_list if 'lambda-patterns.json' in str(call)]
        assert len(json_calls) == 1
        
        # Check Markdown file write
        md_calls = [call for call in mock_file.call_args_list if 'api-endpoints.md' in str(call)]
