...
class TestDynamoDBSchemaExtractor:
    ...
    def test_initialization_success(self, mock_boto3_session):
        ...
        mock_session.assert_called_once_with(
            profile_name="test", region_name="us-east-1"
        )
    ...
    def test_get_field_description(self, mock_boto3_session):
        ...
        # Test unknown field
        assert (
            extractor._get_field_description("unknown_field") == "Field: unknown_field"
        )
    ...
    def test_extract_all_schemas(self, mock_boto3_session):
        ...
        mock_table = Mock()
        mock_resource.Table.return_value = mock_table
        mock_table.scan.return_value = {"Items": [{"id": "123", "name": "Test User"}]}
        ...
        assert (
            schemas["heatherandwesley-users"]["tableName"] == "heatherandwesley-users"
        )
    ...
    def test_generate_markdown_docs(self, mock_boto3_session):
        ...
        test_schemas = {
            "test-table": {
                ...
                "keySchema": {"partitionKey": "id", "sortKey": "timestamp"},
                "fields": {
                    "id": {
                        "types": ["string"],
                        "required": True,
    ...
    def test_save_outputs(self, mock_makedirs, mock_file, mock_boto3_session):
        ...
        json_call = [
            ...
            if "dynamodb-schemas.json" in str(call)
        ][0]
        ...
        md_call = [
            call
            for call in mock_file.call_args_list
            if "field-mappings.md" in str(call)
        ][0]
    ...
    def test_main_success(self, mock_extractor_class):
        ...
        mock_extractor_class.assert_called_once_with(
            profile_name="personal", region="us-east-1"
        )
        mock_extractor.extract_all_schemas.assert_called_once()
        mock_extractor.save_outputs.assert_called_once_with(
            {"table1": {"data": "test"}}
        )
...
class TestEdgeCases:
    ...
    def test_permission_denied(self, mock_session):
        ...
        mock_client.describe_table.side_effect = Exception(
            "User: arn:aws:iam::123456789012:user/test is not authorized to perform: dynamodb:DescribeTable"
        )
