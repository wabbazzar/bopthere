...
class TestAuthHandlerSecurity:
        ...
        def mock_update_item(Key, **kwargs):
            ...
            if username in mock_table.data:
                ...
                if ":last_login" in expression_values:
                    mock_table.data[username]["last_login"] = expression_values[
                        ":last_login"
                    ]
            return True
    ...
    def auth_handler(self, mock_boto3):
        ...
        with patch.dict(
            ...
            spec = importlib.util.spec_from_file_location(
                "auth_handler",
                os.path.join(
                    os.path.dirname(__file__), "../../../../aws/lambda/auth-handler.py"
                ),
            )
    ...
    def existing_user(self, mock_dynamodb_table):
        ...
        password_hash = (
            f"{salt}:{hashlib.sha256(('SecurePass123!' + salt).encode()).hexdigest()}"
        )
        user_data = {
...
class TestLoginEndpoint(TestAuthHandlerSecurity):
    ...
    def test_login_missing_username(self, auth_handler):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/login",
            "body": json.dumps({"password": "SecurePass123!"}),
        }
    ...
    def test_login_invalid_username(self, auth_handler):
        ...
        event = {
            ...
            "path": "/auth/login",
            "body": json.dumps(
                {"username": "nonexistentuser", "password": "SecurePass123!"}
            ),
        }
    ...
    def test_login_invalid_password(self, auth_handler, existing_user):
        ...
        event = {
            ...
            "path": "/auth/login",
            "body": json.dumps(
                {"username": "existinguser", "password": "WrongPassword123!"}
            ),
        }
...
class TestTokenVerification(TestAuthHandlerSecurity):
    ...
    def test_valid_token_verification(self, auth_handler, existing_user, valid_token):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/verify",
            "body": json.dumps({"token": valid_token}),
        }
    ...
    def test_token_in_authorization_header(
        self, auth_handler, existing_user, valid_token
    ):
    ...
    def test_missing_token(self, auth_handler):
        ...
        event = {"httpMethod": "POST", "path": "/auth/verify", "body": json.dumps({})}

        response = auth_handler(event, {})

        assert response["statusCode"] == 401  # 401 is correct for missing auth
        body = json.loads(response["body"])
    ...
    def test_expired_token(self, auth_handler, existing_user):
        ...
        with patch.dict(os.environ, {"JWT_SECRET": "test-secret-key-for-testing"}):
            ...
            expired_token = jwt.encode(
                payload, "test-secret-key-for-testing", algorithm="HS256"
            )

        event = {
            "httpMethod": "POST",
            "path": "/auth/verify",
            "body": json.dumps({"token": expired_token}),
        }
    ...
    def test_malformed_token(self, auth_handler):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/verify",
            "body": json.dumps({"token": "this.is.not.a.valid.jwt.token"}),
        }
...
class TestCORSHandling(TestAuthHandlerSecurity):
    ...
    def test_options_preflight_request(self, auth_handler):
        ...
        event = {"httpMethod": "OPTIONS", "path": "/auth/login"}
...
class TestSecurityVulnerabilities(TestAuthHandlerSecurity):
    ...
    def test_sql_injection_attempts_username(self, auth_handler):
        ...
        for injection_attempt in injection_attempts:
            event = {
                ...
                "path": "/auth/login",
                "body": json.dumps(
                    {"username": injection_attempt, "password": "testpass"}
                ),
            }
    ...
    def test_xss_prevention_in_responses(self, auth_handler):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/login",
            "body": json.dumps({"username": xss_payload, "password": "testpass"}),
        }
    ...
    def test_password_hash_verification(
        self, auth_handler, sample_user_data, mock_dynamodb_table
    ):
        ...
        assert (
            stored_user["password_hash"] != sample_user_data["password"]
        )  # Should be hashed
...
class TestJWTTokenSecurity(TestAuthHandlerSecurity):
    ...
    def test_jwt_token_tampering(self, auth_handler, existing_user, valid_token):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/verify",
            "body": json.dumps({"token": tampered_token}),
        }
    ...
    def test_jwt_wrong_secret(self, auth_handler, existing_user):
        ...
        event = {
            "httpMethod": "POST",
            "path": "/auth/verify",
            "body": json.dumps({"token": wrong_secret_token}),
        }
...
class TestErrorHandling(TestAuthHandlerSecurity):
    ...
    def test_unsupported_http_method(self, auth_handler):
        ...
        for method in unsupported_methods:
            event = {"httpMethod": method, "path": "/auth/login"}
    ...
    def test_invalid_endpoint_path(self, auth_handler):
        ...
        invalid_paths = ["/auth/invalid", "/auth/", "/auth", "/invalid/path", "/"]
