...
class TestAPIFieldConsistency:
    ...
    def _extract_response_fields(self, content: str) -> Set[str]:
        ...
        response_pattern = r'#### (?:Success|Error) Response.*?\n+```json\s*\n(.*?)\n```'
    ...
    def _analyze_lambda_fields(self, tree: ast.AST, code: str) -> Dict[str, Any]:
        ...
        response_pattern = r"'data':\s*(?:decimal_to_float\()?(?:item|response\['Item'\])"
    ...
    def test_dynamodb_schema_matches_lambda_usage(self, dynamodb_schema, lambda_handler_analysis):
        ...
        if unused_fields:
            print(f"Warning: DynamoDB schema defines fields not used in Lambda: {unused_fields}")
    
    def test_api_documentation_matches_lambda_patterns(self, api_documentation, lambda_handler_analysis):
        ...
        lambda_response_fields = lambda_handler_analysis['field_usage']['response_fields']
    ...
    def test_required_fields_properly_documented(self, dynamodb_schema, api_documentation, lambda_handler_analysis):
        ...
        print(f"Info: DynamoDB requires all fields, API requires subset: {lambda_required}")
    
    def test_all_documented_fields_are_used(self, api_documentation, lambda_handler_analysis):
        ...
        assert not unused_fields, (
            f"API documentation includes fields that are never used: {unused_fields}"
        )
    
    def test_no_undocumented_response_fields(self, api_documentation, lambda_handler_analysis, dynamodb_schema):
        ...
        for field in lambda_response_fields:
            if field not in top_level_fields and field not in schema_fields:
                assert field in doc_response_fields, (
                    f"Lambda returns undocumented field: {field}"
                )
    
    def test_field_defaults_match_schema(self, lambda_handler_analysis, dynamodb_schema):
    ...
    def test_timestamp_fields_consistency(self, dynamodb_schema, lambda_handler_analysis):
        ...
        for field in timestamp_fields:
            if field in dynamodb_schema['properties']:
                assert dynamodb_schema['properties'][field]['type'] == 'string', (
                    f"Timestamp field '{field}' should be type 'string'"
                )
        
        # Verify Lambda generates timestamps
        lambda_fields = lambda_handler_analysis['field_usage']['dynamodb_fields']
        assert timestamp_fields.issubset(lambda_fields), (
    ...
    def test_attendance_field_values(self, dynamodb_schema, api_documentation):
        ...
        attendance_examples = dynamodb_schema['properties']['attendance'].get('examples', [])
        assert 'yes' in attendance_examples or 'no' in attendance_examples, (
            "Attendance field should have 'yes'/'no' examples"
        )
        
        # Check API documentation
        doc_content = api_documentation['content']
        assert 'yes|no' in doc_content, (
            "API documentation should specify attendance as 'yes|no'"
        )
    
    def generate_consistency_report(self, dynamodb_schema, api_documentation, lambda_handler_analysis):
        ...
        for field in sorted(schema_fields):
            ...
            report.append(f"  - {field}: {field_type} {'(required)' if required else ''}")
        ...
        for field in sorted(lambda_fields['request_fields']):
            ...
            report.append(f"  - {field} {'(required)' if required else f'(default: {default})'}")
        ...
        for field, info in sorted(doc_fields.items()):
            report.append(f"  - {field}: {info['type']} {'(required)' if info['required'] else '(optional)'}")
        
        # Consistency Issues
        report.append("\nConsistency Analysis:")
        
        # Check for mismatches
        all_fields = schema_fields | set(doc_fields.keys()) | lambda_fields['request_fields']
    ...
    def test_error_response_consistency(self, api_documentation):
        ...
        error_patterns = re.findall(r'#### Error Response.*?\n```json\s*\n(.*?)\n```', 
    ...
    def test_lambda_validation_matches_documentation(self, lambda_handler_analysis, api_documentation):
    ...
    def test_field_naming_conventions(self, dynamodb_schema):
        ...
        for field_name in dynamodb_schema['properties'].keys():
            ...
            assert re.match(r'^[a-z_]+$', field_name), (
                f"Field '{field_name}' contains invalid characters"
            )
    
    def test_optional_fields_have_defaults(self, api_documentation, lambda_handler_analysis):
        """Test that all optional fields have default values in Lambda"""
        # Get optional fields from documentation
        optional_fields = {
            field for field, info in api_documentation['request_fields'].items()
            if info.get('optional', False)
        }
        
        # Get fields with defaults from Lambda
        fields_with_defaults = set(lambda_handler_analysis['field_usage']['field_defaults'].keys())
