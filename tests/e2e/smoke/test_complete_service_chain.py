...
class TestCompleteServiceChain:
    ...
    def teardown_class(cls):
        ...
        if cls.created_items['users'] or cls.created_items['rsvps']:
            ...
            for item_id in cls.created_items['users'] + cls.created_items['rsvps']:
                try:
    ...
    def test_api_gateway_discovery(self):
        ...
        try:
            ...
            assert len(wedding_apis) > 0, "No wedding app API Gateway found in us-east-1"
            ...
            for path in expected_paths:
                assert any(path in rp for rp in resource_paths), f"Path {path} not found in API Gateway"
    ...
    def test_health_endpoint_complete_service_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                assert 'region' in data, "Health response missing region field"
                assert 'services' in data, "Health response missing services field"
                
                # Verify correct region
                assert data['region'] == AWS_REGION, f"Health check reports wrong region: {data['region']}"
                ...
                for table_name in [SERVICES[svc]['table'] for svc in SERVICES.values()]:
                    assert table_name in dynamodb_status, f"Table {table_name} not in health status"
                    assert dynamodb_status[table_name]['status'] == 'active', \
                        f"Table {table_name} not active: {dynamodb_status[table_name]}"
                    assert dynamodb_status[table_name]['region'] == AWS_REGION, \
                        f"Table {table_name} not in {AWS_REGION}"
                
                # Verify Lambda service status
                lambda_status = services['lambda']
                for lambda_name in [SERVICES[svc]['lambda'] for svc in SERVICES.values()]:
                    assert lambda_name in lambda_status, f"Lambda {lambda_name} not in health status"
                    assert lambda_status[lambda_name]['status'] == 'active', \
                        f"Lambda {lambda_name} not active: {lambda_status[lambda_name]}"
                    assert lambda_status[lambda_name]['region'] == AWS_REGION, \
                        f"Lambda {lambda_name} not in {AWS_REGION}"
                        
            elif response.status_code == 404:
                pytest.skip("Health endpoint not implemented yet")
            else:
                pytest.fail(f"Health endpoint returned error: {response.status_code}: {response.text}")
    ...
    def test_complete_rsvp_flow_chain(self):
        """Test complete RSVP flow: submit → verify storage → field validation"""
        api_url = self.test_api_gateway_discovery()
        ...
        assert rsvp_response.status_code in [200, 201], f"RSVP submission failed: {rsvp_response.text}"
        ...
        for field, value in rsvp_data.items():
            ...
            assert item[field] == value, f"Field {field} value mismatch: {item[field]} != {value}"
    ...
    def test_complete_leaderboard_flow_chain(self):
        """Test complete leaderboard flow: submit score → retrieve → verify ranking"""
        api_url = self.test_api_gateway_discovery()
        ...
        for i, score_value in enumerate([1000, 2500, 1500]):
            ...
            scores_submitted.append({
                'score': score_value,
                'character': score_data['character']
            })
        ...
        assert 'total_players' in leaderboard_data, "Leaderboard missing total_players field"
        ...
        assert score_values == sorted(score_values, reverse=True), "Scores not sorted correctly"
    ...
    def test_cross_service_data_consistency(self):
        ...
        game_type, scores, leaderboard_data = self.test_complete_leaderboard_flow_chain()
        
        # Verify user context consistency
        # The username from auth should appear in leaderboard if same user
        username = user_data['username']
        leaderboard_usernames = [s['username'] for s in leaderboard_data['scores']]
        ...
        if user_item:
            user_timestamp = user_item[0].get('created_at', user_item[0].get('timestamp'))
            if user_timestamp:
    ...
    def test_error_handling_across_services(self):
        ...
        error_tests = [
            ...
            },
            {
                ...
                'method': 'GET',
                'data': None,
                'expected_status': [200, 404]  # 200 if it returns empty, 404 if not found
            }
        ]
        ...
        for test in error_tests:
            ...
            if test['method'] == 'POST':
                response = requests.post(test['endpoint'], json=test['data'], headers=headers, timeout=30)
            else:
            ...
            assert response.status_code in test['expected_status'], \
                f"Unexpected status for {test['endpoint']}: {response.status_code}"
    ...
    def test_service_performance_chain(self):
        ...
        assert leaderboard_time < 10.0, f"Leaderboard flow too slow: {leaderboard_time:.2f}s"
...
class TestServiceChainIntegration:
    ...
    def test_service_configuration_consistency(self):
        ...
        for service_name, config in SERVICES.items():
            assert 'lambda' in config, f"Service {service_name} missing lambda configuration"
            assert 'table' in config, f"Service {service_name} missing table configuration"
            
            # Lambda names should follow naming convention
            assert config['lambda'].startswith('heatherandwesley-'), \
                f"Lambda {config['lambda']} doesn't follow naming convention"
            
            # Table names should follow naming convention
            assert config['table'].startswith('heatherandwesley-'), \
    ...
    def test_api_endpoint_consistency(self):
        ...
        assert AWS_REGION in API_GATEWAY_URL or 'us-east-1' in API_GATEWAY_URL, \
        ...
        # This is documentation - actual endpoint testing is done in the main tests
        print(f"Expected endpoints: {expected_endpoints}")
        print(f"API Gateway URL: {API_GATEWAY_URL}")


if __name__ == '__main__':
    # Run with: python -m pytest tests/e2e/smoke/test_complete_service_chain.py -v
