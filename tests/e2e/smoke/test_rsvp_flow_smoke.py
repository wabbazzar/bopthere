...
class TestRSVPFlowE2E:
    ...
    def test_rsvp_submission_success(self):
        ...
        # Validate response
        assert response.status_code in [200, 201], f"RSVP submission failed: {response.text}"
    ...
    def test_rsvp_field_validation(self):
        ...
        if response.status_code in [200, 201]:
            ...
            required_fields = ['name', 'email', 'attendance', 'dietary_restrictions', 
                             'plus_one', 'plus_one_name', 'guest_count', 'comments']
            
            for field in required_fields:
                assert field in stored_item, f"Field {field} not found in stored RSVP"
                assert stored_item[field] == complete_rsvp[field], \
    ...
    def test_rsvp_response_time(self):
        ...
        assert execution_time < 5.0, f"RSVP submission took too long: {execution_time:.2f}s"
...
class TestRSVPIntegration:
    ...
    def test_rsvp_field_consistency_with_frontend_types(self):
        ...
        expected_rsvp_fields = {
            'name', 'email', 'attendance', 'dietary_restrictions',
            'plus_one', 'plus_one_name', 'guest_count', 'comments'
        }
