...
class TestMigrationVerification:
    ...
    def test_dynamodb_tables_in_us_east_1(self, dynamodb_client):
        ...
        for table_name in required_tables:
            assert table_name in existing_tables, f"Table {table_name} not found in us-east-1"
            
            # Verify table is active
            table_info = dynamodb_client.describe_table(TableName=table_name)
            assert table_info['Table']['TableStatus'] == 'ACTIVE', f"Table {table_name} is not active"
    ...
    def test_lambda_functions_in_us_east_1(self, lambda_client):
        ...
        for lambda_name in required_lambdas:
            assert lambda_name in existing_functions, f"Lambda {lambda_name} not found in us-east-1"
            
            # Verify function configuration
            func_info = lambda_client.get_function(FunctionName=lambda_name)
            config = func_info['Configuration']
            assert config['State'] in ['Active', 'LastUpdateStatus'], f"Lambda {lambda_name} is not active"
    ...
    def test_api_gateway_in_us_east_1(self, apigateway_client):
        ...
        assert 'heatherandwesley-api' in api_names, "API Gateway not found in us-east-1"
        
        # Get the API details
        api = next((api for api in response['items'] if api['name'] == 'heatherandwesley-api'), None)
        assert api is not None, "Could not find heatherandwesley-api"
        
        # Test that API has expected resources
    ...
    def test_no_resources_in_us_west_2(self):
        ...
        assert len(wedding_tables) == 0, f"Found DynamoDB tables in us-west-2: {wedding_tables}"
        
        # Check Lambda functions
        west_lambda = west_session.client('lambda')
        west_functions = west_lambda.list_functions()['Functions']
        wedding_functions = [f['FunctionName'] for f in west_functions if f['FunctionName'].startswith('heatherandwesley')]
        assert len(wedding_functions) == 0, f"Found Lambda functions in us-west-2: {wedding_functions}"
        
        # Check API Gateway
        west_apigateway = west_session.client('apigateway')
        west_apis = west_apigateway.get_rest_apis()['items']
        wedding_apis = [api['name'] for api in west_apis if api['name'].startswith('heatherandwesley')]
        assert len(wedding_apis) == 0, f"Found API Gateways in us-west-2: {wedding_apis}"
    ...
    def test_health_endpoint_accessible(self, apigateway_client):
        ...
        try:
            ...
            if health_response.status_code == 200:
                ...
                assert health_data['region'] == 'us-east-1', f"Health endpoint reports wrong region: {health_data['region']}"
    ...
    def test_service_integration_basic(self, apigateway_client):
        ...
        try:
            ...
            assert auth_response.status_code in [200, 400, 401, 403], \
                f"Auth service not responding correctly: {auth_response.status_code}"
                
        except requests.exceptions.RequestException:
            pytest.skip("Auth endpoint not accessible - may need additional configuration")
        
        # Test leaderboard endpoint (basic connectivity test)
        try:
            leaderboard_response = requests.get(f"{api_base}/leaderboard/tetris", timeout=10)
            
            # Should return 200 or proper error, not gateway errors
            assert leaderboard_response.status_code in [200, 400, 404], \
                f"Leaderboard service not responding correctly: {leaderboard_response.status_code}"
                
        except requests.exceptions.RequestException:
            pytest.skip("Leaderboard endpoint not accessible - may need additional configuration")
...
class TestMigrationConsistency:
    ...
    def test_environment_variables_updated(self):
        ...
        for lambda_name in required_lambdas:
            try:
                func_info = lambda_client.get_function(FunctionName=lambda_name)
                env_vars = func_info['Configuration'].get('Environment', {}).get('Variables', {})
                
                # Check that no environment variables reference us-west-2
                for key, value in env_vars.items():
                    if isinstance(value, str):
                        assert 'us-west-2' not in value, \
    ...
    def test_iam_roles_region_consistent(self):
        ...
        try:
            ...
            wedding_roles = [role for role in roles if role['RoleName'].startswith('heatherandwesley')]
            ...
            for role in wedding_roles:
                ...
                try:
                    ...
                    for policy_name in policies['PolicyNames']:
                        ...
                        assert 'us-west-2' not in policy_doc, \
