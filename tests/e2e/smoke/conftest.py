...
def test_rsvp_factory(api_gateway_url, cleanup_tracker):
    ...
    def create_test_rsvp(name_suffix: str = None, **kwargs) -> Dict[str, Any]:
        ...
        if response.status_code in [200, 201]:
            ...
            raise Exception(
                f"Failed to create test RSVP: {response.status_code}: {response.text}"
            )
...
def jwt_token_factory():
    ...
    def create_token(
        username: str = None, role: str = "guest", expires_in_hours: int = 1
    ) -> str:
        import jwt
        ...
        # Use the same secret as the Lambda functions
...
def score_submission_helper(api_gateway_url, jwt_token_factory, cleanup_tracker):
    ...
    def submit_score(
        game_type: str, score: int, character: str = "wesley", username: str = None
    ) -> Dict[str, Any]:
        import requests
        ...
        score_data = {"score": score, "character": character}
        ...
        if response.status_code in [200, 201]:
            ...
            raise Exception(
                f"Failed to submit score: {response.status_code}: {response.text}"
            )
...
class SmokeTestHelpers:
    ...
    def validate_cors_headers(
        response_headers: Dict[str, str], origin: str = None
    ) -> bool:
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(
            ...
            for keyword in ["performance", "concurrent", "chain"]
        ):
